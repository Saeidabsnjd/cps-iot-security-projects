...
# keep it readable: drop the noisy "saved ..." figure-path lines
shown = [ln for ln in lines if not ln.strip().startswith("saved fig")]
text = "\n".join(shown)

n = len(shown)
fig_h = max(4.0, 0.205 * n)
fig = plt.figure(figsize=(12.5, fig_h), dpi=130)
fig.patch.set_facecolor("#0c0c0c")
ax = fig.add_axes([0, 0, 1, 1]); ax.axis("off")
ax.set_facecolor("#0c0c0c")

ax.text(0.010, 0.992, "  modbus reproduction  -  python experiments/run_all.py",
        va="top", ha="left", family="monospace", fontsize=10,
        color="#9cdcfe", transform=ax.transAxes)
ax.text(0.010, 0.965, text, va="top", ha="left", family="monospace",
        fontsize=8.4, color="#d4d4d4", transform=ax.transAxes)
